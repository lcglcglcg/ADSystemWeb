...
document.writeln("  <div style=\"height: 20px;line-height: 20px;\" name=\"twoTimeQList\"><span style=\"color:#0033CC;cursor: pointer;\">今天</span> | <span style=\"color: #000000;cursor: pointer;\">昨天</span> | <span style=\"color: #0033CC;cursor: default;\">最近七天</span> | <span style=\"color: #0033CC;cursor: pointer;\">上周</span> | <span style=\"color: #0033CC;cursor: pointer;\">本月</span> | <span style=\"color: #0033CC;cursor: pointer;\">上个月</span> | <span style=\"color:#0033CC;cursor: pointer;\">上个季度</span> | <span style=\"color:#0033CC;cursor: pointer;\">默认</span>");
...
$G(function(){
	...
	,	day:function(){
		...
		tomorrow:function() {
			...
			now=$G.formatdate(now,'YYYY-MM-dd');
			
			this.setFlash(now,now);
			return now;
		}
	,	yesterday:function(){
	...
	,	ThisMonth:function(lastvalue){
			...
			// 此处因为不了解formate中三个参数的意思 只能按传两个参数的方法 设置本月选择期间
			this.setFlash($G.formatdate(new Date(currentYear,currentMonth + 1,0),'YYYY-MM-dd'),$G.formatdate(firstDay,'YYYY-MM-dd'));
		...
		$G(s).click(function(){
			...
			if(two){
				...
				window.insetTimeLoad=function(o){
					...
					else if(setvalue =="tomorrow")dateFn.tomorrow();
			...
			if (temp&&(temp.indexOf("至") > 0)) {
				temp = temp.split("至");
				
				temp[0] = temp[0].replace(/[\u4e00-\u9fa5]/g, "");
				temp[1] = temp[1].replace(/[\u4e00-\u9fa5]/g, "");
			
				temp = temp[0] + " 至 " + temp[1];
			}
			
			obj.setAtt('bakvalue',$G.Trim(temp));
