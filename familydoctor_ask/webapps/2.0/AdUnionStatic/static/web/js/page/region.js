	...
	window.region=function(){
		...
		,	getRegionSelect=function(){
				...
				r_1_obj.Each(function(){
					...
					// 补齐四个直辖市的数据
					if (num == "1000100000") {	// 北京
						codes = codes.concat(BeijingCodes);
					} else if (num == "1000300000") {	// 天津
						codes = codes.concat(TianjinCodes);
					} else if (num == "1000400000") {	// 重庆
						codes = codes.concat(ChongqingCodes);
					} else if (num == "1000200000") {	// 上海
						codes = codes.concat(ShanghaiCodes);
					}

					if($G.isNumber(num) && ((regleve==2)||regleve!=2)){
						codes.push(num);
					}
				});
				return codes.join(',');
		...
		,	calback:function(JSON){
				...
				$G.Each(function(i,k){
					...
					,	d=function(){
							...
							// 补齐四个直辖市的数据
							if (rD.parentId == "1000100000") {	// 北京
								BeijingCodes.push(code);
							} else if (rD.parentId == "1000300000") {	// 天津
								TianjinCodes.push(code);
							} else if (rD.parentId == "1000400000") {	// 重庆
								ChongqingCodes.push(code);
							} else if (rD.parentId == "1000200000") {	// 上海
								ShanghaiCodes.push(code);
							}
