				...
				,	callbackEach:function(n){
						$G('.TI2D2GHA4T4HL1 .TI2D2GHA4T4HL2 .TI2D2GHA4T4HL3 .TI2D2GHA4T4HL4 .TI2D2GHA4T4HL5 .TI2D2GHA4T4HL6').Each(function(){
							...
							if(dpDefHtml[name]===false){
								...
								this.innerHTML=dpDefHtml[name].img;
						});
					}
				});
			};
...
'  <p style="  margin:0; width:120px; float:left;"> <img src="'+imgUrl+'" style="width:120px; height: 85px;"></p>',
'  <dl style="font-size:12px;line-height:20px; padding:0; margin:0; position:absolute; left:150px; height:85px;">',
'    <dt style="height:25px; line-height:25px; overflow:hidden;color:#1386A5; font-size:14px; text-decoration:none;"><!--title--></dt>',
'    <dd style="margin:0; color:#767676; font-size:12px; height:40px; overflow:hidden;"><!--description1--></dd>',
'    <dd style="position:absolute; right:10px; bottom:0; margin:0; height:20px; text-align:center; width:100%;"><span style="background:#1d9dba; color:#fff;padding:0 5px; display:inline-block; margin-left:10px;">在线咨询</span><span style="background:#dd6800; color:#fff;padding:0 5px;  display:inline-block;margin-left:15px;">立即预约</span></dd>',
'  </dl>',
'  </a></div>'].join("");
			...
			var getPP=function(){
				...
				,	fn:function(d){
					...
					//回调结束 无广告时也执行
				,	callbackEach:function(n){
						$G('div:tj-tab-layout div:filter-layout #BAIDU_SSP__wrapper_u1558431_0').none();
					}
				});
			};
			getPP();
