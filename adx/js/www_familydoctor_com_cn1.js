...
if(!window.adxShow){
	document.writeln('<link rel="stylesheet" type="text/css" href="http://static.adx.haomeit.com/static/jiatingyisheng.com/web/css/focus.css"/>');
	document.writeln('<link rel="stylesheet" type="text/css" href="http://static.adx.haomeit.com/static/jiatingyisheng.com/web/css/jiatingyisheng.css"/>');
	document.writeln('<div class="jtyy_box" style="width:250px; height:250px;">');
	document.writeln('    	<a class="block1 font13 po1_t0f0" href="http://jk.haomeit.com/guke/?familydoctor" name="style1" target="_blank">最好的骨科医院排名</a>');
	document.writeln('        <a class="block1 font16 po1_t0fn" href="http://jk.haomeit.com/baidianfeng/?familydoctor" name="style1" target="_blank">白癜风治疗</a>');
	document.writeln('        <a class="block3 font24 po1_tnf0" href="http://jk.haomeit.com/yangwei/?familydoctor" name="style3" target="_blank">男人阳痿怎么办？</a>');
	document.writeln('        <a class="block4 font23 po1_tn2f0" href="http://jk.haomeit.com/buyunbuyu/?familydoctor" name="style4" target="_blank">不孕不育医院</a>');
	document.writeln('        <a class="block1 font16 po1_tn1fn1" href="http://jk.haomeit.com/zaoxie/?familydoctor" name="style1" target="_blank">性交障碍</a>');
	document.writeln('        <a class="block1 font15 po1_tn1f0" href="http://jk.haomeit.com/yinjingyc/?familydoctor" name="style1" target="_blank">阴茎延长</a>');
	document.writeln('        <a class="block3 font24 po1_tn3f0" href="http://jk.haomeit.com/dx/?familydoctor" name="style3" target="_blank">癫痫病能治愈吗</a>');
	document.writeln('        <a class="block2 font16 po1_tn4fn2" href="http://jk.haomeit.com/baopigc/?familydoctor" name="style2" target="_blank">包皮过长怎么办</a>');
	document.writeln('        <a class="block2 font15 po1_tn5fn3" href="http://jk.haomeit.com/renliu/?familydoctor" name="style2" target="_blank">意外怀孕怎么办</a>');
...
(function(){
	...
	$G.wmImgs=function(selector){
		...
		var showFn=function(o){
				...
				var url=w.url;
				var regExpFn=function(s,k,v){

					var reg=new RegExp('{'+k+'}','ig')
					;
					return s.replace(reg,v);
				};
				var altHtml='<div class="jtyy_qie_box">\
  <div class="l_box_bg"> <a href="{url_1}" target="_blank"><img name="box_bg" src="{src_1}" height="250" width="250"></a>\
    <div class="tm_bg">\
      <div class="tm_btn" name="bgPic">{list}\
      </div>\
    </div>\
  </div>\
</div>';
				var altHtmlA='';//'<a value="{src}" forurl="{url}">{i}</a>';
				var altHtmlList='';
				var show=function(html,isList){
					var WinAltDom=$G('div:jtyy_box');
					if($G.isNaN(html)){
						html=WinAltDom[0].innerHTML;
						/*$G.getjs({
							url:'http://58guahao.haomeit.com/win/images/jiating.js',
							charset:'utf-8',
							cache:false
						});
						WinAltDom[0].style.display='none';
						return;*/
					}
					WinAltDom[0].innerHTML='<a class="close_link"><img src="http://static.adx.haomeit.com/static/jiatingyisheng.com/web/images/close_03.jpg" width="12" height="12"/></a>'+html;
					WinAltDom.WinAlt({
							position:'fixed',
							isRight:true,
							isBottom:true,
					...
					$G('a:close_link',WinAltDom[0]).click(function(){
						WinAltDom.display().none();
					});
					if(!isList || 1==1)return;
				...
				if(isAdxShow!==false){
					...
					if(o.data.length<1){
						isAdxShow(setArray,'img');
						return;
					}
					$G.Each(function(i){
						var j=o.data[i];
						if(!j)return;
						var imgUrl=o.targetUrl+'&link=100&url='+function(){
							var url='http://'+j.destinationUrl;
							return $G.Trim(encodeURIComponent('http://'+(url.replace((new RegExp('http://','ig')),''))));
						}()+'&creative='+j.creativeId
						,	webIM=$G.isNaN(j.webIM)?'':function(){
								return o.targetUrl+'&link=2&url='+function(){
									var url='http://'+j.webIM;
									var val=url
									,	repFn=function(url,name){
											var s=$G.getQuery.call(url,name)
											,	u=url
											;
											if(s){
												s=name+'='+s;
												u=wmReplace(url,s,s+'_haomeitid_'+toKen);
											}else{
												u = u + ( /\?/.test( s.url ) ? "&" : "?" );
						...
						,	imgHref=wwwImg+j.filePath
						,	addJson={}
						,	uesrPic='http://images.adx.haomeit.com/HospitalImgFile/'+j.act+'.JPG?notcache='+(new Date().getTime())
						;
						addJson['imgUrl']=imgUrl;
						addJson['href']=imgHref;
						addJson['webIM']=webIM;
						addJson['uesrPic']=uesrPic;
						setArray.push(addJson);
					},wmObj.cabin.split(","));
					isAdxShow.call(o.data,setArray,'img');
					return;
				}
				/*if($G.isNaN(o.data)){
					o.data=[{targetUrl:'http://www.913wen.com/nanke/?99'}];
				}*/
				try{
				...
				$G.Each(function(i){
					var j=o.data[i];
					if(!j)return;
					var hrefUrl=o.targetUrl
					,	wIm=''
					,	name=this+''
					,	url=o.targetUrl+'&link=100&url='+function(){
						var url='http://'+j.destinationUrl;
						return $G.Trim(encodeURIComponent('http://'+(url.replace((new RegExp('http://','ig')),''))));
					}()+'&creative='+j.creativeId
					;
					if(i==0){
						altHtml=regExpFn(altHtml,'url_1',url);
						altHtml=regExpFn(altHtml,'src_1',wwwImg+j['filePath']);
					}
					altHtmlList+=function(){
						var str=altHtmlA;
						str=regExpFn(str,'url',url);
						str=regExpFn(str,'src',wwwImg+j['filePath']);
						str=regExpFn(str,'i',(i+1));
						return str;
					}();
				},wmObj.cabin.split(","));
				if(o.data.length<2)altHtmlList='';
				show(regExpFn(altHtml,'list',altHtmlList),altHtmlList);
				try{
...
/*(function(){
	var param={
		cabin:'TI2SVB154T4HK6,TI2AOFT84T4H15,TI2D2GHA4T4HL0'
	,	param:{
			trust:7086
		,	style:'www_familydoctor_com_cn'
		}
	,	callback:function(n){
...
(function(){
	$G.wm=function(){
		...
		,	param={
			...
			,	cb:'window.set'+uuid
			,	style:'www_familydoctor_com_cn_Text'
			,	trust:7086
			}
		,	www='http://show.adx.haomeit.com'
		...
		,	leftHtml='<div></div>'.replace(/{www}/ig,www_static+'/')
		,	regitHtml='<div></div>'.replace(/{www}/ig,www_static+'/')
		,	showFn=function(o){
				o=o||{};
				...
				$G.Each(function(i){
					...
					$G.Each(function(aaa,k){
						...
						if(isL=='L'){//长广告变量替换
							if(k=='append'){//附加创意
								if(this.length>0 && !$G.isNaN(this[0].title)){
									...
									$G.Each(function(appendI){
										...
										$G.Each(function(m,ak){
											...
											if(ak=='title' || ak=='destinationUrl')
												addAppendJson[ak]=reVal.val;
								...
								if(this.length>0 && !$G.isNaN(this[0].token)){
									...
									addJson['guaHaoDisplay']=reVal.val;
									wmHtml=wmHtml.replace(reVal.key,reVal.val);
								}else{
						...
						if(!$G.isNaN(val)){
							addJson[(reg.key+'').replace('/<!--','').replace('-->/gi','')]=reg.val+'';
				...
				if(isAdxShow!==false){
					for(var i=0,n=setArray.length;i<n;i++){
						$G.Each(function(k,ak){
							var linkStr=clickLink[ak];
							if(ak=='webIM' || ak=='destinationUrl' || ak=='uesrUrl'){
								if(setArray[i]){
									setArray[i][ak]=setArray[i][ak]+'&link='+clickLink[ak];
								}
							}
							if(ak=='uesrUrl')setArray[i]['uesrPic']=setArray[i][ak];
						},setArray[i]);
					}
					isAdxShow.call(o.data,setArray,'text');
					return;
				}
				try{
					wmObj.callbackEach&&wmObj.callbackEach.call(o,o.data.length);
...
if(!window.wmTextShow){
	window.wmTextShow=function(){
		//广告位标识：标识顺序就是排名顺序，TL为长广告位标识，TS为短广告标识
		this.cabin='TL2NJVSO33BC1K,TL2SG68J33BC1Q,TL2P4PE933BC27';
		//广告显示后回调方法 n 为当前广告总数
