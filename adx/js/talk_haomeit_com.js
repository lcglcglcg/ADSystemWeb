...
var hmCode='%3F87ed271a6abcd8693f604f7cbaa3cb88';
if($G.mobile){
	hmCode='%3F87ed271a6abcd8693f604f7cbaa3cb88';
	$G('#mobile')[0].style.display='';
}else{
	$G('#pc')[0].style.display='';
}

var shengyuliuliang=function(){
	var surl=window.document.location.pathname.replace(/\//ig,'')
	,	www='http://jiankang.58.com/wenda/m/mquestion/'
	;
	document.location=(www+surl+'/');
};
...
(function(){
	...
	$G.wm=function(){
		...
		,	url=www+'/pass?'+function(){
				...
				var styles='talk.haomeit.com1';
				if($G.mobile){
					styles='m.talk.jk.haomeit.com1';
				}
				return styles;
		...
		,	showFn=function(o){
				...
				if(o.data.length<1){
					...
					,	frmUrl='http://58guahao.haomeit.com/url.aspx?dirname='+function(){
							var surl=window.document.location.pathname.replace(/\//ig,'')//((window.document.location+'').replace('http://talk.haomeit.com','')).replace(/\//ig,'')
							,	surl=surl.split("?")
							,	surl=function(){
									if(surl.length>0){
										surl=surl[0];
				...
				$G.Each(function(i){
					var name=this+''
					,	getJson=function(){
							var rk=window.getRandom(0,o.data.length)||0;
							rk=0;
							return o.data[rk]||''
						}()
