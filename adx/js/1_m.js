...
(function(){
	windowDom=document.getElementsByTagName;
	var divCode=$G.uuidFast().replace(/-/ig,'');
	document.getElementsByTagName=function(type){
		var list=windowDom.call(this,type);
		if(type=='div'){
			var tmpList=new Array();
			if(!$G.isNaN(window.cabin)){
				$G('.'+window.cabin.replace(/,/ig,' .')).Each(function(){
					$G('div',this).setAtt('wmcode',divCode);
				});
				$G.Each(function(i){
					var code=$G(this).getAtt('wmcode');
					if(divCode!=code)
						tmpList.push(this);
				},list);
			}
			if(tmpList.length>0)list=tmpList;
		}
		return list;
	};
	$G.wm=function(){
		...
		,	url=www+'/pass?device=1&'+$G.param(param)+'&cabin='+(wmObj.cabin||'')						// 广告位标识
		,	listHtml='<div style="overflow:hidden;font: normal 100% Helvetica, Arial, sans-serif; line-height:100%; position:relative;">\
  <div style="float:left; height:64px;overflow: hidden; margin-right:78px;">\
    <div style="height:22px; line-height:22px; font-size: 1.0em;font-weight:bold;overflow: hidden;">{title}</div>\
    <div style="height:22px; line-height:22px;font-size: 0.75em; color:#101211;overflow: hidden;">{description1}</div>\
    <div style=" display:inline-block;height:20px;line-height:20px;font-size: 0.8em;color:#5da7c4;overflow: hidden;padding-right:25px; background:url({www}web_pic.jpg) no-repeat center right;">{mobile_display_url}</div>\
  </div>\
  <div style="display:{webimDisplay};float:right; position:absolute; right:0;">\
    <div style="width:68px; height:64px; margin-left:2px;position:relative; display:table-cell; vertical-align:middle; text-align:center;">{webIM}</div>\
  </div>\
</div>'.replace(/{www}/ig,www_static+'/')
		,	clickUrlTest='<a href="{url}" style="{style}" target="_blank">{test}</a>'
		,	showFn=function(o){
				...
				,	clickList={
						'title':{
							'link':11
						,	'css':'color:#000; text-decoration:none; cursor:pointer;'
						}
					,	'webIM':{
							'link':12
						,	'css':'border:none; text-decoration:none; cursor:pointer;'
						,	'key':'webIM'
						,	'val':'<span style="*position:absolute;top:50%;"> <img src="{www}btn.gif" width="68" height="23" style=" border:none;*position:relative; top:-50%;left:-50%;"/> </span>'.replace(/{www}/ig,www_static+'/')
						}
					,	'description1':{
							'link':13
						,	'css':'color:#101211; text-decoration:none; cursor:pointer;'
						}
					,	'mobile_display_url':{
							'link':14
						,	'css':'color:#5da7c4;text-decoration:none; cursor:pointer;'
						}
					}
				,	wrParam=function(key,val,isExp){
						var clickArray=clickList[key]
						,	creative=this
						;
						val=val||'';
						if(!isExp && clickArray){
							var url=targetUrl+'&link='+clickArray.link+'&url='+function(){
								var url='http://'+creative.mobile_destination_url;
								if(key=='webIM'){
									url=creative.webIM||'';
								}
								url=url+toKen;
							...
							,	reValUrl=wrParam('url',url,true)
							,	reValTitle=wrParam('test',function(){
									if(clickArray.val)
										return clickArray.val;
									else return val;
								},true)
							,	reValColor=wrParam('style',clickArray.css,true)
							;
							if(clickArray.key)
								key=clickArray.key;
							val=clickUrlTest.replace(reValUrl.key,reValUrl.val).replace(reValTitle.key,reValTitle.val).replace(reValColor.key,reValColor.val);
						}
						return {
				...
				$G.Each(function(i){
					...
					$G.Each(function(aaa,k){
						...
						,	val=this
						;
						if(!$G.isNaN(val)){//短广告变量替换
							if(k=='webIM'){
								var DisplayVal=$G.isNaN(val)||$G.isObj(val)?'none':''
								,	DisplayReg=wrParam.call(getJson,'webimDisplay',DisplayVal)
								;
								wmHtml=wmHtml.replace(DisplayReg.key,DisplayReg.val);
								reg=wrParam.call(getJson,k,'');
							}
