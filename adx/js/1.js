...
(function(){
	$G.wm=function(){
		...
		,	url=www+'/pass?style=58.com2&'+$G.param(param)+'&cabin='+(this.cabin||'')// 广告位标识
		,	clickLink={
				'title':1
			,	'webIM':2
			,	'append_0':3
		...
		,	showFn=function(o){
				...
				$G.Each(function(i){
					...
					$G.Each(function(aaa,k){
						...
						if(k=='act'){
							reg=wrParam.call(getJson,'uesrUrl',www_static.replace(/images/,'')+'HospitalImgFile/'+val+'.JPG?notcache='+(new Date().getTime()));
						}
						if(!reg){
