window.headerEnd(function(){
		...
		var recharge_alert=function(){
				...
				,	sub=function(){
						...
						,	ckdata:function(data){
								...
								,	accountingDocumentDiscount=data['DISCOUNT']//返点比例
								,	accountingDocumentDiscountObj=$G('.DISCOUNT',recharge_alert[0])
								,	money= data['MONEY'] = $G.toDecimal2(parseInt($G(".MONEY", recharge_box)[0].value || 0) * (1 + parseInt($G(".DISCOUNT", recharge_box)[0].value || 0) * 0.01)) + ""
								...
								if(!(/^-?[0-9]\d*$/.test(accountingDocumentDiscount)) && parseInt(accountingDocumentDiscount) < 0 && parseInt(accountingDocumentDiscount) >= 100){
									window.msg.call(accountingDocumentDiscountObj[0],{title:'提示信息',msg:'返点比例必须是0-100的整数'});
									return;
								}
								...
								data["BRAND_TYPE"] = data["BRAND_TYPE"][0]; 
								return data;
							}
						,	callback:function(JSON){
								if(window.wm.msg(JSON.CODE,false)){
			...
			if(name=='demo'){
				...
				recharge_alert.show(function(){
					...
					$G('li span',fd[0]).Each(function(){
						...
						if(!$G.isNaN(name)){
							this.innerHTML=d[name];
							if( name=='BALANCE'||name=='INVEST'){
								this.innerHTML=$G.FormatNumber(d[name]);
							}
						}
					});
					$G('input',fd[0]).val(value);
					$G('.MONEY .ACCOUNTING_DOCUMENT_REMARK .DISCOUNT .ACCOUNTING_DOCUMENT_NUMBER .ACCOUNTING_DOCUMENT_FILE',recharge_alert.obj).Each(function(){
						this.value='';
					});
				});
	...
	var recharge_box = $G('#recharge_alert')[0];
	
	$G(".DISCOUNT", recharge_box)[0]["onkeyup"] = function() {
		if(!(/^-?[0-9]\d*$/.test(this.value)) || parseInt(this.value) < 0 || parseInt(this.value) > 100){
			window.msg.call(this, {title:'提示信息',msg:'返点比例必须是0-100的整数'});
			this.value = "";
			return;
		}
		
		$G(".virtualMoney", recharge_box)[0].innerHTML = (parseInt($G(".MONEY", recharge_box)[0].value || 0) * (1 + parseInt(this.value || 0) * 0.01)).toFixed(2);
	};
	
	setInterval(function() {
		$G(".virtualMoney", recharge_box)[0].innerHTML = $G.toDecimal2(parseInt($G(".MONEY", recharge_box)[0].value || 0) * (1 + parseInt($G(".DISCOUNT", recharge_box)[0].value || 0) * 0.01));
	}, 200);
