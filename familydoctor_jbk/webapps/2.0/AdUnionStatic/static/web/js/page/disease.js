window.disease=function(){
	...
	,	ifType=function(v){
			return v.substr(0,1)<2?true:false;
		}
	,	jsonList={
			'leve1':{}
		,	'leve2':{}
	...
	,	diseaseLeve_2=$G('.diseaseLeve_2',diseaseList[0])
	...
	,	selectChange=function(fnd){
			...
			,	text=$G(this).getseltext()[ifType(value)?'diseaseLeve_1':'diseaseLeve_2']
			;
	...
	get(function(JSON,codeType){
		var isTrue=ifType(codeType);
		if(isTrue){
			...
			diseaseLeve_2.delallsel();
			diseaseLeve_2.addsel('选择或输入定向科室','0');
			$G.Each(function(i,t){
				var id=this+''
				,	leve=getCodeLeve(t,id).ls
				;
				if(leve==1){
					diseaseLeve_2.addsel(id,t);
				}
			},JSON);
			diseaseLeve_2[0].onchange=selectChange;
		}
	...
	$G('.diseaseSelectAll .diseaseSelectDifferent .diseaseSelectClear',diseaseList[0]).click(function(){
		var labelList=$G('label',diseaseList[0])
		,	type=this.name
		;
	...
	var isDiseaseLeveDisp=function(){
		return $G('.diseaseLeveDisp')[0].style.display==''?true:false;
	};
	return {
		getlist:getlist
	,	openList:openList
	,	getid:function(){
			if(isDiseaseLeveDisp()){
				if(diseaseLeve_1.getselvalue()['diseaseLeve_1']=='0')return '';
			}else{
				if(diseaseLeve_2.getselvalue()['diseaseLeve_2']=='0')return '';
			}
			var id=new Array()
			,	dgIds={}
	...
	,	setid:function(id){
			...
			if(ifType(id))
				...
				diseaseLeve_2.setselvalue(pid);
			window.selDom.resall();
			selectChange.call(ifType(id)?diseaseLeve_1[0]:diseaseLeve_2[0],function(){
				var selectListTmp={}
				,	leveIds={}
				;
