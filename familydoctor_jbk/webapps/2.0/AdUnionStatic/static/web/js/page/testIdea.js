	...
	window.testIdea=function(){
		...
		,	ideaAction=function(){
				...
				,	demo=function(){
						...
						$G('.CREATIVE_DESTINATION_URL',idea[0]).bd({on:'keyup',callback:function(){
							var valueLen=$G.len(this.value);
							if(valueLen>1016){
				...
				,	subIdea=window.subAjax('#step3_test',{
							ckdata:function(data){
								...
								,	destinationUrl=$G('.CREATIVE_DESTINATION_URL',idea[0])
								...
								,	mobileDestinationUrl=$G('.MOBILE_DESTINATION_URL',idea[0])
								...
								mod['CREATIVE_DESTINATION_URL']='http://'+(mod['CREATIVE_DESTINATION_URL'].replace(/(http:\/\/)/ig,""));
								if($G.isNaN(mod['CREATIVE_APPEND_ID'])) {mod['CREATIVE_APPEND_TYPE']=1; mod["UPDATE_TYPE"] = parseInt(mod["UPDATE_TYPE"] == "" ? 0 : mod["UPDATE_TYPE"]) + 32;}
								else {mod['CREATIVE_APPEND_TYPE']=2; }
								if(!$G.isNaN(mobileDestinationUrl[0].value))
									mod['MOBILE_DESTINATION_URL']='http://'+(mod['MOBILE_DESTINATION_URL'].replace(/(http:\/\/)/ig,""));
								return mod;
							}
				...
				,	domShow=function(type,JSON){
						...
						$G('.CREATIVE_ID .CAMPAIGN_ID .GROUP_ID .CREATIVE_NAME .CREATIVE_DESCRIPTION .CREATIVE_DESTINATION_URL .CREATIVE_DISPLAY_URL .MOBILE_DESTINATION_URL .MOBILE_DISPLAY_URL .IMG_PATH',idea[0]).Each(function(){
							var gObj=$G(this)
							,	name=gObj.getAtt('name')
							;
				...
				,	callback:function(JSON){
						...
						try{
							...
							for(var i=0;i<n;i++){
								...
								v.MOBILE_DESTINATION_URL=v.MOBILE_DESTINATION_URL.replace(/(http:\/\/)/g,"");
