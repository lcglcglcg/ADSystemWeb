	...
	window.brands=function(){
		...
		,	list=window.subAjax('.brandList',{
			...
			,	ckdata:function(){
					...
					data['REGION_SERIALIZE']=window.region.getRegion(true);
					...
					if(brandType==0){
						var CREATIVE_STATUS=$G('.CREATIVE_STATUS',brandSelect[brandType])
						;
						data['STATUS']=1;
						data['CREATIVE_STATUS']=CREATIVE_STATUS[0].value;
			...
			,	callback:function(JSON){
					...
					try{
						...
						for(var i=0;i<n;i++){
							...
							if(brandType!=3){
								if(v.CREATIVE_STATUS!=4)tools+=view;
					...
					$G('.creativeIni',DataList[0]).click(function(){
						...
						$G('.CREATIVE_STATUS',addCreative[0])[0].value=j.CREATIVE_STATUS;
						window.ajax({
						...
						,	calback:function(JSON){
								...
								$G('.CREATIVE_STATUS',addCreative[0])[0].value=JSON['CREATIVE_STATUS'];
