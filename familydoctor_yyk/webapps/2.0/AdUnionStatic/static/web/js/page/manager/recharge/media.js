				...
				if(n>0){
					...
					$G.Each(function(i,k){
						...
						if(!$G.isNaN(v) && !notDoName[v]) {
							if (v.toLowerCase().indexOf("dsp") >= 0) {
								tempArr.push(i);
								if (v.toLowerCase().indexOf("text") >= 0) {
									MEDIA_ID.addsel("DSP-文字",k);
								} else if (v.toLowerCase().indexOf("img") >= 0) {
									MEDIA_ID.addsel("DSP-图片",k);
								} else {
									MEDIA_ID.addsel(v,k);
								}
							}
