	...
	return $G.upflie({
		success:function(file, serverData){
			...
			if(ini.upid=='upIdeas_0' && JSON.CREATIVE_IMG_SIZE!='170X140'){
				window.msg.call(dom[0],{msg:'图片尺寸必须为170X140'});
				return;
			}
			if(ini.upid!='upIdeas_0' && JSON.CREATIVE_IMG_SIZE!='120X90'){
				window.msg.call(dom[0],{msg:'图片尺寸必须为120X90'});
				return;
			}
...
var model=function(d,isTrue){
	...
	if(!d['CREATIVE_STATUS']||d['CREATIVE_STATUS']==4){
...
var demoFn=function(JSON){
	if(!JSON.CREATIVE_STATUS)return;
	...
	$G('.GUAHAO_URL',demo[0])[0].href='http://'+(JSON.GUAHAO_URL.replace(/(http:\/\/)/ig,""));
	$G('.DESTINATION_URL .TITLE .WEB_SITE .TELEPHONE',demo[0])[0].href='http://'+(JSON.DESTINATION_URL.replace(/(http:\/\/)/ig,""));
